...
from ommyfitness import settings
...
def signup(request):
    if request.method == 'POST':
        ...
        try:
            ...
            subscription = Subscription.objects.create(user=user, plan_id=1, start_date=current_date,
                                                       end_date=current_date, )
            subscription_serializer = SubscriptionSerializer(subscription).data
            ...
            return JsonResponse({'status': 'error', 'message': 'Error while creating user', 'status_code': 400},
                                status=400)
    else:
...
def login(request):
    if request.method == 'POST':
        admin_tag = request.POST.get('admin')
        if CustomUser.objects.filter(phone_number=request.POST.get('phone_number')).exists():
            ...
            if user is None:
                ...
                if admin_tag == 'true' and not user.is_staff:
                    return JsonResponse(
                        {'status': 'error', 'message': 'Not authorized to log in as an admin.', 'status_code': 401},
                        status=401)
                else:
                    return authentication_result(user)
        return JsonResponse(
            {'status': 'error', 'message': "This phone_number doesn't exist. Create a new account or check your SMS",
             'status_code': 404},
            status=404)
    else:
...
def authentication_result(user):
    ...
    return JsonResponse({'status': 'success', 'message': 'Success, welcome back', 'token': str(token),
                         'username': str(user.username), 'user_id': str(user.id),
...
def fetch_users(request):
    ...
    if token_key:
        ...
        return JsonResponse(
            {'status': 'success', 'message': 'Users fetched successfully.','data': list(user_serializer), 'status_code': 200}, status=200)
